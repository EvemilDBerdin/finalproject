...
try { 
    ...
    $data['query'] = '';
    $data['query'] .= "SELECT * FROM users WHERE  role != 'student' and flag=0 ";

    if ($data['db']->getStatus()) {
        if (!empty($_POST['search']['value'])) {
            $data['query'] .= 'and school_id LIKE "%'.$_POST["search"]["value"].'%" ';
            $data['query'] .= 'OR role LIKE "%'.$_POST["search"]["value"].'%" '; 
            $data['query'] .= 'OR email LIKE "%'.$_POST["search"]["value"].'%" ';
        }
